...
>>>>>>> Stashed changes
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      console.error("No pet yet:", error);
    }
...
>>>>>>> Stashed changes
      ...
      setPet((prev) => {
        ...
        const allUnlocked =
          getUnlockedAccessories(newXp).map((a) => a.id);

        const newUnlocked =
          allUnlocked.filter((id) => !prevUnlocked.includes(id));
        ...
        return {
          ...
          dailyProgress: isNewDay
            ? minutes
            : prev.dailyProgress + minutes,
          unlockedAccessories: [
            ...new Set([...prevUnlocked, ...newUnlocked])
          ],
        };
      });
<<<<<<< Updated upstream
